...
VERSION     : v1.14（R14）
DATE        : 2026-08-26（v1.0 制定）／ 2026-08-27（v1.2〜v1.10 改定）／ 2026-08-28（v1.11・v1.12 改定）／ 2026-08-29（v1.13 改定）／ 2026-08-30（v1.14 改定）
工程        : HP-ONBOARDING-4A ／ -4A-R1（AI Sales 分離・Operations 境界確定）
              ...
              ／ -4H-3（本体コードの本番配置・APP_ROOT 解決経路の実機確定）
              ／ **-4H-4（preflight 専用CLI の実装・本改定）**
...
### 9.10 preflight 専用環境（v1.12 で新設・4H-R0／v1.14 で実施方法を確定・4H-4）
...
v1.12 は preflight の**規則**だけを定め、
「何をどう流すか」を定めていなかった。
4H-4 の調査で、実行経路が**実装されていない**ことが分かったため、
専用 CLI を設けて確定する。

| # | 規則 |
|---|---|
| 1 | 実行は **CLI のみ**。Web 経路・cron・一時公開PHP を作らない |
| 2 | 位置は **`APP_ROOT/preflight/` に固定**。CLI は任意の絶対パスを引数で受け取らない |
| 3 | コマンドは `preflight:init` / `run` / `status` / `remove` / `verify-empty` の5つ |
| 4 | **正式 `intake-config.php` を require しない**（overrides だけで Config を組む）。<br>設定ファイル内のコードを実行する経路を作らない |
| 5 | **正式設定または正式DB（journal / wal / shm / ratelimit / backups を含む）が存在すれば、<br>`init` / `run` は順序違反として実行しない**。<br>`status` / `remove` / `verify-empty` にはこの条件を課さない（撤去のため） |
| 6 | 鍵（`ip_hmac_key` / `enc_key`）は CLI が**2つ別々に生成**し、隔離領域の設定へ書く。<br>**argv・出力・ログへ出さない** |
| 7 | 管理者ID・パスワード・hash は **`run` のプロセス内で生成**し、設定ファイルへ書かない。<br>hash は **Argon2id**。使えなければ STOP（BCRYPT へ落とさない）。**出力しない** |
| 8 | 通知は **`NullNotifier` を明示注入**する。通知先・差出人を設定しない |
| 9 | 作成は **umask 0077 / ディレクトリ 700 / 設定ファイル 600**。<br>設定ファイルは**排他的に新規作成**し、既存なら**上書きしない** |
| 10 | 作成前に**字句検査・絶対パス・禁止セグメント・最寄りの既存親までの symlink 検査**を行う。<br>作成後に **realpath 一致検査**を行い、差し替えを検出する |
| 11 | 通し確認は**本物の App / AdminApp / Guard / RateLimiter** を通す。<br>テスト専用の偽物だけで通過させない |
| 12 | `run` の**直前に正式領域の baseline** を取り、**直後に比べる**。<br>比べるのは一覧・配置物（src / bin）と安定ファイルの SHA-256・<br>出現してはならないものである。**固定の期待値を持たない** |
| 13 | **ログは別扱い**。本文をハッシュしない。<br>**消失**または**権限 600 の崩れ**は STOP。<br>サイズの増減は**記録に留める**（ローテーション等の外部要因を誤検出しない） |
| 14 | `init` が失敗しても**自動で削除しない**。<br>撤去は status → dry-run → **代表承認** → apply の順で行う |
| 15 | 出力に**絶対パス全文を載せない**（`<APP_ROOT>/preflight` と表示する） |
...
| 4 | **本番バックアップ先の具体パス**／**APP_ROOT の解決経路** | §9.5・§10.11。世代・削除方針・命名・cleanup・復元確認は v1.11（4G）で、配置境界は v1.12（4H-R0）で確定した。<br>`auto_prepend_file` の実機確認は v1.13（4H-3）で完了し、本番は**祖先探索方式**を採用した（§10.11）。<br>**preflight の実施方法は v1.14（4H-4）で確定**した（§9.10）。<br>残るのは **XServer 上の正確な絶対パス（`backup_dir`）・権限・`SQLite3::backup()` の実測**である。<br>★**§9.9 の条件が揃うまで `backup_policy_confirmed` を真にしない** | 4H（実機確認） |
...
| **v1.14（R14）** | 2026-08-30 | 改定（HP-ONBOARDING-4H-4・代表承認）。**§9.10 preflight の実施方法を確定**した。v1.12 は preflight の**規則**を定めたが、「何をどう流すか」を定めておらず、4H-4 の調査で**実行経路が実装されていない**（`PreflightArea` は確認と削除だけを持ち、作成も通し確認も行わない）ことが判明した。**専用 CLI**（`preflight:init` / `run` / `status` / `remove` / `verify-empty`）を設け、位置を **`APP_ROOT/preflight/` に固定**して任意の絶対パスを引数で受け取らないこと、**正式 `intake-config.php` を require しない**（overrides だけで Config を組み、設定ファイル内のコードを実行する経路を作らない）こと、**正式設定または正式DBが存在すれば `init` / `run` を順序違反として実行しない**（`status` / `remove` / `verify-empty` には課さない）ことを定めた。鍵は CLI が**2つ別々に生成**して隔離領域の設定へ書き、管理者ID・パスワード・hash（**Argon2id**。使えなければ STOP）は**`run` のプロセス内だけで生成**して設定ファイルへ書かず、**argv・出力・ログへ一切出さない**ことを明記。通知は **`NullNotifier` を明示注入**し、通知先・差出人を設定しない。作成は **umask 0077 / ディレクトリ 700 / 設定 600** で、設定ファイルは**排他的に新規作成し、既存なら上書きしない**。作成前の**字句検査・symlink 検査**と作成後の **realpath 一致検査**を必須化した。通し確認は**本物の App / AdminApp / Guard / RateLimiter** を通す。**正式領域の不変検査**は、固定の期待値を持たず `run` の直前に baseline を取って直後に比べる方式とし、一覧・配置物（src / bin 全件）と安定ファイルの SHA-256・出現してはならないものを見る。**ログは別扱い**とし、本文をハッシュせず、**消失と権限 600 の崩れだけを STOP** とし、サイズの増減は記録に留める（ローテーション等の外部要因を誤検出しない）。`init` 失敗時は**自動削除せず**、status → dry-run → 代表承認 → apply の順とした。**§12.2-4 の残課題から preflight の実施方法を消し込んだ**。**新しいDB表は追加していない**（`PRAGMA user_version` は 4 のまま・JSON 11分類・回答スキーマ版 1 も不変）。**受付API・店舗入力画面・必須契約・書き出し契約・保持削除・バックアップの規則は一切変更していない。** |
