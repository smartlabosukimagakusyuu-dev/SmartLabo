...
- **Smart Labo Salon LP（正式URL `salon.html`）の法務表示方針（2026-09-01 / 代表決定）**：
  ...
  **資料請求**（`contact.html?type=docs&topic=salon#form`）の 2 つ。
  契約成立は別の契約・申込フローで行い、**契約確定前の最終確認・同意段階**で
  契約条件および必要な法定表示を確認できる方式を正式方針とする。
  導線：公開LP → 相談・資料請求 → 契約/申込フロー → 最終確認 → 法定表示確認 → 同意 → 契約確定。
  この方針に合わせ、フッターの該当項目はリンクを持たない `<span>` のままとし、
  表記を「特定商取引法に基づく表記」→「**法定表示は契約時に確認**」へ改めた
  （存在しない特商法ページの URL は作らない）。
  ※ **本記述は実装方針の記録であり、法的適法性の判断ではない。**
  ※ **将来、LP から直接契約申込みを完了できる構造へ変更する場合は、
    広告表示を含め法務要件を再監査すること。**
- **契約フロー側の将来要件（Salon / 未実装）**：契約確定ボタンの直前で、
  少なくとも以下を利用者が確認できること：
  契約対象サービス/プラン、利用期間・提供回数等、販売価格（役務の対価）、
  初期費用、その他追加料金、支払方法、支払時期、サービス提供開始時期、
  最低契約期間、更新条件、解約条件、解約方法/連絡方法、事業者情報、必要な法定表示。
  さらに、**契約内容を確認したこと / 法定表示を確認したこと / 必要な規約へ同意したこと**を
  記録できる設計とする。（今回は契約画面・決済・DB の実装は行っていない）
