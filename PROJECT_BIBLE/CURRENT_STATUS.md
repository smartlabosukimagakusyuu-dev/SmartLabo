...
Homepage Version：2.8（プライバシーポリシー・利用規約ドラフト作成：privacy.html/terms.htmlを「準備中」から正式ドラフトへ、専門家確認前提の注記付き）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：正式リリース前 最終整備(CEO承認2026-07-14)のStep1・Step2完了。会社概要ページ・法務ページドラフトを新設。未確定の会社情報・特定商取引法の該当性はいずれも断定せず「CEO確認待ち」「専門家確認待ち」と明示。Step3(問い合わせフォームの送信基盤選定・セキュリティ設計確定)以降はCEO承認待ち
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.8 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。`privacy.html`/`terms.html`が「準備中」から正式ドラフト（専門家確認前提）へ更新 |
...
| Current Task | **正式リリース前 最終整備 Step1・Step2完了。** Step1：会社概要ページ(`company.html`)・会社情報一元管理(`js/company-info.js`)を新設。Step2：`privacy.html`/`terms.html`を正式ドラフト化（事業者情報/取得情報/利用目的/Cookie/安全管理措置/特定商取引法の該当性等を記載、専門家確認前提の注記付き）。未確定の会社情報・特定商取引法の該当性は断定せず「CEO確認待ち」「専門家確認待ち」と明示 |
| Next Task | Step3(問い合わせフォームの送信基盤最終選定・セキュリティ設計の詳細確定、CEO承認)はCEO承認待ち。Step4以降(バックエンド実装)も同様。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち |
