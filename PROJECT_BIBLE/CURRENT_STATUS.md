...
```
Project Bible Version：5.8
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.6（pricing.htmlを「Smart Labo Configurator」へ刷新：利用規模→基本プラン→利用モジュール→追加オプション→おすすめ構成確認の5Stepシミュレーター）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：Smart Labo Configurator実装完了(CEO承認2026-07-13)。Pricing Philosophyを①利用規模×②基本プラン×③利用モジュール×④追加オプションの4層構造へ改訂し、Company Brain/CRM/AI Assistant等の中核機能はプラン標準搭載(選択不可)、真に個別性の高い拡張のみを追加オプションとして区別。ホームページでの選択内容はcontact.htmlへ自動引き継ぎ。見積書/契約書自動生成・CRM連携・バックエンドAPIは未実装(スコープ外)
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.6 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。`pricing.html`が「Smart Labo Configurator」(5Stepシミュレーター)へ刷新 |
...
| Current Task | **Smart Labo Configurator実装完了。** `pricing.html`を①利用規模→②基本プラン→③利用モジュール表示→④追加オプション→⑤おすすめ構成確認の5Stepシミュレーターへ刷新。[12_Pricing_Philosophy.md](12_Pricing_Philosophy.md)を4層構造(利用規模/基本プラン/利用モジュール/追加オプション)へ改訂し、Company Brain/CRM/AI Assistant等の中核機能はプラン標準搭載として区別。選択内容はcontact.htmlへ自動引き継ぎ。Company OS「Pricing Manager」自体は引き続き設計のみで実装は別Task承認待ち |
| Next Task | PROJECT_BIBLE・WEBSITE・BUSINESS_STRATEGY更新分のpushはCEO確認後。Company OS「Pricing Manager」の実際のコード実装(料金表/見積書/契約書/営業資料の自動生成)は、契約書の法的正確性を要するためスコープを別Taskとして提案しCEO承認を得てから着手する。営業テンプレート／Appointment Templateの追加分・他業種テンプレートの追加、Smart AI Router公開API化も同様 |
