...
```
Project Bible Version：6.2
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.8（ホームページ本体の見た目に変更なし。問い合わせフォーム送信基盤をXServer(PHP)前提へ変更、xserver-form/を新設しWEBSITE/api/[Vercel版]は削除）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/（正式ドメインsmartlaboworks.comへの切替はCEOアクション待ち）
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：正式リリース前 最終整備(CEO承認2026-07-14)のStep1・2完了、送信基盤はCEO判断によりVercelからXServer(PHP)へ変更してStep4を再実装。xserver-form/(GitHub Pages非公開)にPHPバックエンドを実装し、ローカルPHP環境で22件のユニットテスト+実HTTP統合テストを実施済み。XServerサブドメイン追加・DNS設定・SMTPアカウント作成はCEOアクション待ちのためStep5(contact.html実接続・本番デプロイ)は未着手
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.8 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。見た目の変更なし。問い合わせフォーム送信基盤はXServer(PHP)前提へ変更（`xserver-form/`新設、未デプロイ） |
...
| Current Task | **正式リリース前 最終整備 Step1・Step2完了、送信基盤をXServer(PHP)へ変更してStep4を再実装完了。** Vercel/Node.js版(旧Step3・4)はCEO判断により撤回・削除。新設`xserver-form/`(GitHub Pages非公開)にPHPバックエンドを実装。SQLiteによるレート制限永続化、依存ライブラリなしの最小SMTPクライアント、CSRF検証の脆弱性対策込みで実装し、ローカルPHP環境で22件のユニットテスト＋実HTTP統合テストを実施 |
| Next Task | Step5(`contact.html`/`js/pages.js`の実送信対応・本番デプロイ)はCEOアクション待ち：XServerサブドメイン`form.smartlaboworks.com`追加・DNS設定・無料独自SSL発行、`info@smartlaboworks.com`メールアカウント作成・SMTP接続情報確認、reCAPTCHA登録（正式公開直前）が前提。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち |
