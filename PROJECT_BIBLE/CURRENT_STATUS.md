...
```
Project Bible Version：5.4
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.4(Hero Experience Upgrade v3。AI Control Centerを静的UIからLive Dashboardへ進化。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/)
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：Hero AI Control CenterをLive Dashboard化(CEO承認2026-07-12)。Router呼吸発光・接続ラインの光の流れ・処理中ローテーション・Company Brain状態変化・KPIカード4種を追加し、Heroコピーの主役を「会社を動かすAI。」へ。pushはCEO確認後
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | **2.4 — GitHub Pagesで公開中**([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。CEO承認「Hero Experience Upgrade Version3」により、AI Control Centerを静的UIから常時アニメーションするLive Dashboardへ進化。詳細は下記「Hero Experience Upgrade v3」を参照 |
...
| Current Task | **Hero Experience Upgrade v3完了。** レイアウトは変更せず、AI Control Centerを「AIが今まさに働いている」印象を与えるLive Dashboardへ進化。Router呼吸発光・接続ラインの光の流れ・処理中ステータスのローテーション(2.6秒ごと)・Company Brain状態変化(参照中→更新中→学習完了)・KPIカード4種(本日のAI処理件数/Company Brain登録数/契約レビュー件数/問い合わせ返信数)を追加。Heroコピーは「会社を動かすAI。」を主役(54px)、「Smart Labo Works」を副題(20px)へ。背景に控えめな粒子とゆっくり動く光を追加。Hero最下部に「Company Brainを見る」スクロール誘導を新設 |
| Next Task | ホームページ5コミット分のpushはCEO確認後。営業テンプレート／Appointment Templateの追加分（followup/thanks/contract等）・他業種テンプレートの追加、および将来のSmart AI Router公開API化はCEO承認後に着手 |
