...
```
Project Bible Version：5.9
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.7（会社概要ページ新設・会社情報表示統一：company.html/privacy.html/terms.html新設、全ページのフッター・nav・OGPを統一）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：正式リリース前 最終整備(CEO承認2026-07-14)のStep1完了。会社概要ページ・会社情報一元管理・法務ページ雛形を新設。代表者/設立/所在地/電話番号/メールアドレス/事業内容はPROJECT_BIBLE上も未確定のため「CEO確認待ち」と明示(推測掲載なし)。Step2(プライバシーポリシー・利用規約ドラフト)以降はCEO承認待ち
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.7 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。会社概要ページ(`company.html`)・法務ページ雛形(`privacy.html`/`terms.html`)を新設し、全ページのフッター・nav・OGPを統一 |
...
| Current Task | **正式リリース前 最終整備 Step1完了。** 会社概要ページ(`company.html`)・会社情報一元管理(`js/company-info.js`)・法務ページ雛形(`privacy.html`/`terms.html`)を新設。壊れていた`#mission`アンカーとフッター著作権表記の英語/正式名不一致を是正。代表者/設立/所在地/電話番号/メールアドレス/事業内容はPROJECT_BIBLE上も未確定のため「CEO確認待ち」と明示し推測掲載していない |
| Next Task | Step2(プライバシーポリシー・利用規約の正式ドラフト作成、特定商取引法表記の要否整理)はCEO承認待ち。Step3以降(問い合わせフォームの送信基盤選定・実装)も同様。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち |
