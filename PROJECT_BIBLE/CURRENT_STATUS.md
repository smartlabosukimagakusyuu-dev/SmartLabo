...
## v1.0 Release Checklistへの全面拡充(2026-07-14 CEO指示)

CEOより「Release Candidate 2完了後、正式リリース前の『v1.0 Release Checklist』を作成してください」との指示があり、[61_Release_Checklist.md](61_Release_Checklist.md)をCEO指定の18項目（ホームページ／問い合わせフォーム／SSL／メール送受信／スマホ表示／PC表示／SEO／OGP／favicon／robots.txt／sitemap.xml／404ページ／Google Search Console登録／Google Analytics登録／PageSpeed／GitHub Release／更新履歴／既知の制限事項）へ全面拡充した。

**新規追加した主な項目**：
- **PC表示**：デスクトップ幅(1440px基準)でのレイアウト確認を独立項目化(既存確認内容を反映、✅)
- **404ページ**：独立項目化(既存確認内容を反映、✅)
- **Google Search Console登録**：CEOのGoogleアカウントでの登録作業が必要なため、Claude Codeでは実施不可であることを明記(❓⏸)
- **Google Analytics登録**：現時点でホームページにアクセス解析は一切導入していない。導入するかどうかはまずCEO判断が必要であり、導入する場合は`privacy.html`の「Cookie・アクセス解析について」の章（現在「導入していない」と明記）を先に更新する必要があると明記(❓)
- **既知の制限事項（Known Issues）**：v1.0公開時点で意図的にスコープ外としている、または今後の対応が必要な10項目（会社情報未確定・法務ページドラフト・reCAPTCHA未設定・Analytics未導入・レート制限の実装方式・CSS/JS未minify・AIチャットのデモ制約・他業種テンプレート未実装・Company OS等の非公開機能）を整理

「リンク」「Console Error」は新設の「ホームページ」章へ統合。項目別ステータスまとめ（✅9／⏸6／❓3）と、全項目確認完了後に「v1.0 Release」として提出する運用（3章）を新設した。
...
## 銀行提出用事業計画書 Version 1.0(2026-07-15 CEO指示、SmartLabo repo `DOCUMENT/FINANCE/`新設)

CEOより「PROJECT_BIBLE／CURRENT_STATUS／CLAUDE.md／BUSINESS_STRATEGY／PRODUCT_REQUIREMENTS／Brand Guideline最新版を確認したうえで、銀行提出用の事業計画書Version1.0を作成してください」との指示があった。ホームページの正式リリース準備（上記各章）とは独立した別系統の成果物。

**作成物**: [DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.0.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.0.pptx)（PowerPoint、PDF提出可能品質）および同フォルダの`.pdf`版。CEO指定の13章構成（表紙／Vision／会社概要／市場課題／サービス概要／競争優位性／ビジネスモデル／市場戦略／売上計画／資金使途／開発ロードマップ／代表者プロフィール／結び）で作成し、ブランドカラー(Navy/Smart Blue/White)・正式ロゴ・タイポグラフィをホームページ・PROJECT_BIBLEと統一した。

**参照した一次情報**: [00_Foundation/](00_Foundation/README.md)のMission/Vision/Value/Brand Identity/Company Story、[12_Pricing_Philosophy.md](12_Pricing_Philosophy.md)（ビジネスモデル章の根拠）、`smartlabo-works/PRODUCT_REQUIREMENTS.md`・`PRODUCT_BOUNDARY.md`（サービス概要・競争優位性章の根拠）、[63_Post_Launch_Roadmap.md](63_Post_Launch_Roadmap.md)・[00_Foundation/10_Future_Roadmap.md](00_Foundation/10_Future_Roadmap.md)（開発ロードマップ章の根拠）。`smartlabo-works/CLAUDE.md`・`BUSINESS_STRATEGY.md`（現時点はスタブで料金・財務項目は未着手）も確認済み。

**推測・捏造をしなかった項目（CEOの実記入が必要）**: 代表者名・設立年月・所在地・資本金・代表者の経歴/創業ストーリーは、PROJECT_BIBLE内に正式情報が一切存在しない（`js/company-info.js`・`99_CEO_MEMORY/01_Founding_Story.md`とも未記入）ため、スライド内で赤字の「CEO確認待ち」「[CEO記入]」表示として明示し、値を推測・創作していない。銀行提出書類における虚偽記載リスクを避けるため、本セッションの一貫方針（事実と推測を分ける）を厳格に適用した。

**CEO確認を得て試算した項目**: 「売上計画（3年間）」章はCEO指示「銀行向けに現実的な数値を提案してください。無理な成長予測は避けること」に基づく試算(Year1:3社/約150万円の先行投資期→Year3:40社/約3,500万円・黒字化)であり、前提条件（月額平均単価想定）と見直し前提を脚注に明記。「資金使途」章は絶対額を定めず、開発費35%/サーバー15%/営業20%/広告15%/運転資金15%の配分比率のみを図解し、具体的な調達希望額はCEO判断待ちである旨を脚注に明記した。

**ファイル管理**: [DOCUMENT/FINANCE/README.md](../DOCUMENT/FINANCE/README.md)の命名規則に従い、実際に金融機関へ提出する版が確定した際は`_submitted`を付したファイル名で確定版を残すことを推奨する。
...
## 経営計画書 Version 1.0（社内検討用）(2026-07-15 CEO追加指示、SmartLabo repo `DOCUMENT/FINANCE/`新設)

CEOより「銀行提出版とは別に、Partner Program・9プラン・継続課金・紹介ネットワークを考慮した実際の経営目標としての『経営計画版』を作成してほしい。費用は固定費・変動費に分類し、特にサーバー費・AI API費・Partner Program報酬を分離して現実的な試算へ修正し、利益率も再計算してほしい」との指示を受け新規作成。**銀行提出用事業計画書とは明確に別の、社外非公開のドキュメント**（表紙に「CONFIDENTIAL — 社内検討用（社外非公開）」と明記）。

**作成物**: [DOCUMENT/FINANCE/SmartLabo_ManagementPlan_v1.0.pptx](../DOCUMENT/FINANCE/SmartLabo_ManagementPlan_v1.0.pptx)（全9章：表紙／本書の位置づけ／導入プラン前提[9プラン価格グリッド]／顧客ミックスと想定ARPU／売上計画[経営目標]／コスト構造:固定費／コスト構造:変動費／利益計画・利益率／まとめ）。

**位置づけ**: 新規導入目標(6社→20社→50社)は銀行提出版と共通とし、収益・コストのモデリング精度と成長ドライバーの織り込み方を変えることで数値を差別化。銀行提出版は「保守的な数値」、本書は「Partner Program・9プラン・継続課金・紹介ネットワークを織り込んだ実力値」と明確に位置づけた。

**①9プラン価格グリッド**: [12_Pricing_Philosophy.md](12_Pricing_Philosophy.md)の①利用規模(Small/Medium/Enterprise)×②基本プラン(Lite/Standard/Premium)の3×3=9構成に、経営計画試算用の想定月額(Small-Lite ¥30,000〜Enterprise-Premium ¥300,000)を新たに仮置き(正式価格は個別見積りのまま未確定であることを明記)。

**②顧客ミックスとARPU**: 新規獲得顧客の構成比が年々上位プラン・大規模企業へシフトする想定のもと、想定ARPU(月額)をYear1 ¥50,000→Year2 ¥68,000→Year3 ¥85,000と設定。

**③売上計画(経営目標)**: 売上高をYear1 550万円・Year2 2,900万円・Year3 7,200万円とし、銀行提出版(380万円/1,700万円/4,500万円)との比較表を併記。差分の主因(上位プラン比率の織込み／Partner Program稼働・アップセル／Enterprise層浸透・紹介複利)を明記。

**④コスト構造の固定費・変動費分類**: CEO指示どおり2層に分類。固定費(人件費・基本管理費)はYear1 650万円→Year2 1,100万円→Year3 1,820万円。変動費は**サーバー費・AI API費・Partner Program報酬・その他変動費の4項目に分離**し、Year1 150万円→Year2 740万円→Year3 2,050万円(Partner Program報酬はYear1は制度開始前のため0円、Year2以降は紹介経由新規契約額×15%[初年度]＋更新契約額×10%で計上)。

**⑤利益計画・利益率の再計算**: 営業利益=売上高−固定費−変動費で算出し、Year1 △250万円(営業利益率△45.5%、銀行提出版の△750万円より小さい損失)、Year2 1,060万円(36.6%、Partner Program稼働により黒字化)、Year3 3,330万円(46.3%)。

**品質確認**: pptxgenjsで新規構築し、スキーマ検証・全9スライドの内容確認・LibreOfficeでのレンダリングによる目視QAを実施。初回レンダリングで(1)「本書の位置づけ」章の比較表がフッターと衝突する不具合、(2)「利益計画・利益率」章のグラフで営業利益(万円)と営業利益率(%)のデータラベルがYear3で重なり判読不能になる不具合、の2件を発見。(1)は行間隔の圧縮、(2)は第2軸(営業利益率)の表示範囲を調整して解消した。

---

## 銀行提出用事業計画書 Version 1.2（収支計画・利益計画ブラッシュアップ）(2026-07-15 CEO追加指示、SmartLabo repo `DOCUMENT/FINANCE/`)

CEOより「現実的で説得力のある利益計画へ修正し、銀行担当者が『この会社なら2年目で黒字化できそう』と思える資料へ改善してほしい」との追加指示。**作成物**: [DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.2.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.2.pptx)（全19章、v1.1の16章から3章増加）。v1.0・v1.1ファイルは比較・追跡用に同フォルダへ保持。

**重要な前提の明確化（v1.1からの変更点）**: v1.0／v1.1では6社→20社→50社を「累計導入社数」として扱っていたが、CEOより「Year2＝Year1契約企業の更新＋新規20社」「Year3＝既存契約更新＋新規50社」という明示的な計算式が示されたため、**6/20/50社を各年の「新規契約数」として再定義**し、継続率(Year1→2を83%、Year2→3を90%と提案)を掛けた更新契約数と合算する形で累計契約数(6社→25社→73社)を算出し直した。

**①料金前提の反映**: 平均契約単価月額15万円(年間契約・年間180万円)、初期導入費10万円/社(導入時のみ)を新たに設定。

**②現実的な契約時期を反映した売上計算**: 新規契約は年間を通じて段階的に契約される前提(Year1平均稼働約5ヶ月/社、Year2約6.5ヶ月/社、Year3約7ヶ月/社)とし、更新契約は満額12ヶ月分で計上。年間売上高：Year1 510万円→Year2 3,050万円→Year3 9,900万円。

**③費用の3分類（固定費・変動費・開発費）**: CEO指示どおり3分類へ再編。①固定費(人件費・家賃・通信費・保険・管理費)：730万円→1,560万円→3,055万円。②変動費(サーバー費・AI API費・Partner Program報酬・広告/営業/導入支援費)：129万円→503万円→1,513万円。③開発費(初期開発・機能追加・保守改善を内訳表示)：550万円→500万円→700万円。

**④サーバー費の現実的な修正**: 従来「サーバー・インフラ費」として一括計上していたものを、CEO指示どおりXServer本体・バックアップ・監視費（合計25万円→43万円→78万円、大幅に現実的な水準へ下方修正）と、利用企業数に比例する変動費として分離したAI API費(25万円→150万円→500万円)に分割。

**⑤Partner ProgramをCACとして整理**: Partner Program報酬(0円→120万円→570万円)を単なる費用ではなく顧客獲得コスト(CAC)として位置づけ、Smart Labo Partner Programページに「CAC比較」card(Partner Program経由 約15万円/社 vs 広告経由(参考値) 約28万円/社)を新設し、広告費削減効果を可視化した。

**⑥利益計画：Year2黒字化を達成**: 営業利益はYear1 △899万円(先行投資期)→**Year2 487万円(営業利益率16.0%、黒字化)**→Year3 4,632万円(46.8%)。「無理な利益計画にはしないこと」「利益率よりも実現可能性を優先」というCEO指示に沿い、Year2は大幅な黒字ではなく堅実な黒字達成にとどめた。

**⑦銀行向け説明の追加**: 「利益計画」ページにCEO指定の5点(なぜ2年目黒字か／利益率改善理由／固定費と変動費の違い／紹介制度による営業効率化／継続課金による利益積み上げ)を箇条書きで明記。

**⑧新設「利益が改善する理由」ページ**: CEO指定の図解(初年度:開発投資→2年目:契約更新→Partner Program→広告費削減→利益改善→3年目:契約積み上げ→ストック収益→利益拡大)を3段の縦フロー図として実装。

**品質確認**: スキーマ検証・全19スライド内容確認・LibreOfficeでの目視QAを実施。初回レンダリングで(1)売上計画ページの新規/更新の積み上げ棒グラフでYear1のゼロ値ラベルと売上高ラベルが重なる不具合、(2)利益計画グラフのYear3付近でのラベル重なり、を発見。(1)は積み上げ棒グラフを廃止し累計契約数の単純棒グラフへ簡素化(内訳は右側の表に集約)、(2)は第2軸の表示範囲調整でそれぞれ解消した。

**要CEO確認事項**: 本書(銀行提出版v1.2)は6/20/50社を「新規契約数」として再定義したが、[経営計画書Version 1.0](../DOCUMENT/FINANCE/SmartLabo_ManagementPlan_v1.0.pptx)は旧解釈(累計導入社数)のまま未更新であり、**両書の前提が一時的に不整合な状態**にある。経営計画書側も同様の新規/更新モデルへ更新するかはCEO確認・指示待ち。

---

## 銀行提出用事業計画書 Version 1.3（収益構造・利益計画ブラッシュアップ）(2026-07-15 CEO追加指示、SmartLabo repo `DOCUMENT/FINANCE/`)

CEOより「『利益率が高い理由』ではなく『利益率が改善していく事業構造』を銀行・投資家・パートナーへ説明できる資料へ進化させてほしい」との追加指示。**作成物**: [DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.3.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.3.pptx)（全21章、v1.2の19章から2章増加）。v1.0〜v1.2ファイルは比較・追跡用に同フォルダへ保持。契約前提・導入目標(6/20/50社の新規契約数解釈)はv1.2から変更なし。

**①収益モデルの再定義**: 「Smart Labo Worksは受託開発会社ではなくSaaS・Platform Businessであり、導入企業数が増えても売上と同じ割合で原価が増えない」という前提を明示。

**②サーバー費・AI APIの再分類**: CEO指示どおり、XServer本体・バックアップ・監視・**ドメイン・メール**を①固定費へ整理(30万円→45万円→65万円、v1.2の変動費25/43/78万円から固定費へ付け替え・現実的な水準へ修正)。**AI API費のみ**を利用企業数に比例する②変動費として残す(25万円→150万円→500万円、金額はv1.2から変更なし)。営業活動費(40万円→100万円→200万円)も①固定費へ移動。結果、固定費計は800万円→1,705万円→3,320万円、変動費計は64万円→360万円→1,235万円に再編。

**③開発費の再定義**: 「1社ごとの個別開発ではなく、共通プラットフォーム開発・テンプレート整備・機能追加・保守改善への投資」として内訳を明示(合計額はv1.2の550/500/700万円から変更なし)。

**④利益計画の再計算**: 費用再分類の結果、費用合計は1,414万円→2,565万円→5,255万円(v1.2からごく僅かな変動)。営業利益はYear1 △904万円→**Year2 485万円(営業利益率15.9%、黒字化)**→Year3 4,645万円(46.9%)。

**⑤新設「Platform Business Model」ページ**: CEO指定の7要素(①共通プラットフォーム②業種別テンプレート③AI Router④Company Brain⑤ストック収益⑥Partner Program⑦顧客獲得コスト(CAC)改善)を、「全顧客が共有する基盤」層と「低コストで顧客を獲得しストック収益へ」層の2段構成で図解。

**⑥新設「なぜ利益率が高まるのか」ページ**: CEO指定7点の説明文に加え、**固定費対売上高比率がYear1 156.9%→Year2 55.9%→Year3 33.5%と急速に低下する**ことを示す表を追加し、「固定費の比重低下が利益率改善の主因」であることを数値で裏付け。銀行向け(返済可能性・継続性・再現性)と投資家向け(スケーラブルなPlatform Business・Recurring Revenue・原価増加が緩やかなストック型ビジネス)の訴求点を併記。

**⑦Partner Programの位置づけ再強調**: 「利益が改善する理由」ページを**「利益率が改善する理由」**へ改題し、CEO指定どおり2年目のフローに「テンプレート活用」ノードを追加(契約更新→テンプレート活用→Partner Program→広告費削減→利益率改善の5ステップ)。Partner Programページにも「営業費ではなく顧客獲得コスト(CAC)改善施策」である旨を明記する一文を追加。

**品質確認**: スキーマ検証・全21スライド内容確認・LibreOfficeでの目視QAを実施。初回レンダリングで「なぜ利益率が高まるのか」ページの固定費比率カード内の注記テキストが次のカードとはみ出し重なる不具合を発見し、カード高さと内部余白の再調整で解消した。

**要CEO確認事項**: [経営計画書Version 1.0](../DOCUMENT/FINANCE/SmartLabo_ManagementPlan_v1.0.pptx)は依然として旧解釈(6/20/50社=累計導入社数、サーバー費を変動費として計上)のまま未更新であり、本書(v1.3)との前提不整合が継続している。固定費・変動費の分類方針(サーバー費を固定費へ)も経営計画書には未反映。

---

## 銀行提出用事業計画書 Version 1.4（収支計画の再修正：代表1名・自宅兼事務所を前提とした実態反映）(2026-07-15 CEO追加指示、SmartLabo repo `DOCUMENT/FINANCE/`)

CEOより「現在の固定費・開発費は実態より高く設定されている。代表1名でスタート・社員採用なし・自宅兼事務所・SaaS型・共通プラットフォーム・テンプレート再利用・XServer利用を前提に、Smart Labo Worksの実際の事業モデルに沿った収支へ修正してほしい」との追加指示。**作成物**: [DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx)（全21章、構成はv1.3から変更なし、費用計画・利益計画の中身のみ全面改訂）。v1.0〜v1.3ファイルは比較・追跡用に同フォルダへ保持。売上高・契約前提(月額15万円・6/20/50社の新規契約数解釈)はv1.2/v1.3から変更なし。

**費用構造を4カテゴリへ再編**: CEO指定の分類に合わせ、①固定費(運営費)②技術基盤費(サーバー・AI API)③開発費(保守・改善)④顧客獲得費(CAC)の4分類へ再編。

| カテゴリ | Year1 | Year2 | Year3 | 算出根拠 |
|---|---|---|---|---|
| ①固定費(運営費) | 50万円 | 180万円 | 480万円 | 通信・保険・管理費・予備費(Year1: 50万円、CEO指定基準どおり)＋人件費(Year1は代表1名・無報酬のため0円、Year2はパートタイム支援を年後半6ヶ月導入[月18.3万円]、Year3は正社員1名通年+パートタイム継続) |
| ②技術基盤費 | 40万円 | 152万円 | 498万円 | XServer本体・ドメイン・バックアップ・監視(固定、8万円→12万円→18万円)＋AI API利用料(利用企業数に比例、32万円→140万円→480万円、CEO指定どおり唯一の顧客数比例費目) |
| ③開発費(保守・改善) | 80万円 | 100万円 | 130万円 | 外部委託(特定機能の部分開発)・開発ツール利用料・テンプレート整備・機能追加/保守の4項目。ゼロから作る前提ではなく、既存基盤への追加投資として算出 |
| ④顧客獲得費(CAC) | 20万円 | 180万円 | 685万円 | Partner Program報酬(紹介経由新規契約額×15%[初年度]+更新契約額×10%、Year1は制度開始前のため0円)＋広告費・導入支援費(新規契約数に比例) |
| **費用合計** | **190万円** | **612万円** | **1,793万円** | |

v1.3(固定費800万円→1,705万円→3,320万円、変動費64万円→360万円→1,235万円、開発費550万円→500万円→700万円、費用合計1,414万円→2,565万円→5,255万円)から**大幅に圧縮**。主因は、7名規模の採用計画を前提としていた人件費(v1.3で最大2,600万円)を、代表1名+段階的な最小限の採用計画(Year3でも正社員1名+パート)へ修正したこと。
...
**引き続き未解消の要CEO確認事項**: [経営計画書Version 1.0](../DOCUMENT/FINANCE/SmartLabo_ManagementPlan_v1.0.pptx)は依然として旧い費用構造・旧解釈のまま未更新であり、本書(v1.4)との前提不整合がさらに拡大している。銀行提出版と経営計画版のどちらを正としてすり合わせるか、要CEO判断。

---

## Smart Labo Platform v1.0 全体アーキテクチャ(2026-07-16 CEO追加指示)

CEOより「Smart Labo Platform v1.0 全体アーキテクチャ設計」の追加指示があった。目的は、Smart Labo Works全体のシステム構成を一枚で理解できる形にまとめ、**銀行・パートナー説明・採用資料・投資家説明・社内設計書のすべてで使える正式資料**とすること。コンセプトは「会社を動かすAI。」— Smart Labo Worksは単なるAIチャットではなく、企業全体を支援するAI Platformであることを伝える設計とした。**実装は行わず、設計資料のみを作成した。**

### Git構成の統一(2026-07-16 CEO決定)

Smart Labo Works関連のリポジトリ／プロジェクトの役割を、以下の3つに正式統一した。

| リポジトリ | 役割 |
|---|---|
| `smartlabo-website` | ホームページ(GitHub Pages) |
| `smartlabo-works` | 製品本体(ログイン後のSaaS) |
| `smartlabo-platform` | AI・API・認証・Company Brain等の共通基盤(2026-07-16新設) |

### 作成した成果物(8点)

CEO指定の①〜⑧すべてを作成した。

1. **Smart Labo Platform全体構成図**：Layer1(利用者)〜Layer6(Database)を1枚のMermaid図でまとめた
2. **レイヤー図**：6レイヤーそれぞれの役割・現状(実装済み／将来)を表形式で整理
3. **データフロー図**：フローA(ホームページ訪問者・Public AI経由)とフローB(ログイン後・Company Brain/CRM経由)の2系統をSequence図で整理。両者が完全に分離していることを明示
4. **AI Router構成図**：`smartlabo-works`(Sprint3 Task1)と同じ設計パターンであることを図解
5. **Public AIとCompany Brainの違い**：利用者・役割・参照知識・アクセス権限・実装場所を比較表で整理。Public AIはCompany Brainへアクセスしないことを明記
6. **将来ロードマップ図**：Version1.0(現在、ホームページ・Public AI・お問い合わせ)→Version2.0(Company Brain・CRM・Meeting AI・OCR統合)→Version3.0(Partner Platform・API公開・Marketplace・他業種展開)
7. **README更新**：`smartlabo-platform/README.md`を新規作成
8. **PROJECT_BIBLE更新**：新設[13_Smart_Labo_Platform_Architecture.md](13_Smart_Labo_Platform_Architecture.md)に会社公式記録としての要約版を記録。[00_Foundation/08_SmartLaboWorks_Concept.md](00_Foundation/08_SmartLaboWorks_Concept.md)(v3.2→v3.3)に相互参照を追加。[README.md](README.md)(v7.8→v7.9)にVersion管理・折り込み構成を反映

技術的な構成図・データフロー図・AI Router構成図の詳細は、別リポジトリ`smartlabo-platform/ARCHITECTURE.md`を正とする(PROJECT_BIBLE側は会社公式記録としての要約に留める設計)。銀行・パートナー・投資家向けの提示用として、HTML版のプレゼンテーション資料も別途用意した。

**ステータス：CEO承認済み（2026-07-16）。Version1.0正式版として採用。** 次のステップとして`api.smartlaboworks.com`のDNS設定・本番デプロイに着手する(実施タイミングはCEO指示待ち)。
...
| **WEB-SALES-1B〜5B** | **Liteでの販売基盤実装(2026-08-09・Lite側feature branchのみ。本番変更0)** | 本コミット |
...
**WEB-SALES-1の要点（2026-08-09・調査と設計のみ／本番変更0）:**
- 一次情報は[docs/reviews/WEB_SALES_1_CONTRACT_AUTOMATION_AUDIT.md](../docs/reviews/WEB_SALES_1_CONTRACT_AUTOMATION_AUDIT.md)。Website(`master` `1252347`／`origin/website-v3` `3ef57db`)とLite(`smartlabo-works-lite` `28e2c60`)を読み取りのみで実測
- **判定: 条件付きGo。** 設計と「既存欠陥の修正工程(WEB-SALES-1B)」はGo、WEB-SALES-2以降の実装着手と本番公開はNo-Go
- **最重要の発見:** 14番/15番が「実装済み」と記録するSALES-2/3/3Rの実体は**凍結リポジトリ`smartlabo-works`側**にあり、現行の正式コードベース`smartlabo-works-lite`には申込・招待・契約状態・決済・メールのサーバー実装が**1つも存在しない**(migration 001〜016のみ／routes・repositoriesに該当なし／`stripe`依存なし)。本工程では指示により旧リポジトリを一切参照していないため、移植可否は未確定
- **仕様矛盾を検出:** 本工程の指示書は「決済成功後に会社作成」、14番v3.0/15番v2.0(2026-07-29代表決定)は「申込直後に環境作成→payment_required→決済でactive」。**環境先行方式(SSOT側)の採用を推奨**
- **販売開始前に必須のNo-Go 3件(Lite既存欠陥):** ①会社・利用者を停止してもセッションが破棄されず最大8時間業務APIを使える ②契約人数上限が警告のみでサーバー強制されない ③ログイン試行制限が存在しない
- **法務が最大のスケジュールリスク:** 特定商取引法に基づく表記ページが未作成、`terms.html`はサイト利用条件のみで契約期間・支払・解約・返金を定めていない、キャンペーン規約も未作成
...
**WEB-SALES-1B〜4の要点（2026-08-09・すべてLiteのfeature branchのみ／本番変更0）:**
- 実装はすべて正式製品 `smartlabo-works-lite`。SmartLabo(Website)は無変更、旧`smartlabo-works`も無変更（読み取り参照のみ・コードのコピー0）
- **WEB-SALES-1B** `15d6be8` セキュリティ基盤。①停止・解約の即時反映（毎リクエストでDB確認・セッション破棄）②契約人数のサーバー側強制（BEGIN IMMEDIATEで同時追加も超えない）③ログイン試行制限（IP/メール/IP+メールの3軸・DB永続・生値を保存しない）④CSRF・Origin防御（許可Origin完全一致＋セッション結び付きトークン）。あわせて停止予定(suspend_scheduled)がログインできない既存不具合を修正
- **WEB-SALES-2** `02b6c8f` Web申込〜会社環境自動作成。★メール認証が終わるまでcompanies/usersを作らない。応答からメール登録有無を判別できない共通応答。トークンはSHA-256のみ保存・GETでは消費しない。provisioningは1トランザクション。migration 018で`contract_status`と`signup_applications`を追加
- **WEB-SALES-3** `97ec04e` 認証メール本送信。Node標準net/tlsのみでSMTP実装（依存追加なし）。設定不足でcaptureへ落とさない。送信結果を分類コードで記録。★contract_started_atを「支払い成立時に確定」へ是正
- **WEB-SALES-3S** 実機疎通完了。XServer SMTP(465/暗黙TLS/AUTH LOGIN)で1通のみ送信し受理を確認。DMARC(p=none)反映済み・SPF/DKIM設定ありを確認。**認証リンク先は`https://lite.smartlaboworks.com`が正**（`app.smartlaboworks.com`は旧製品のミライエβ版であることを実測で判明・誤設定を未然に防止）
- **WEB-SALES-4** `46b1ec2` Stripe決済・Webhook・契約状態管理。詳細は[docs/reviews/WEB_SALES_4_STRIPE_CONTRACTS.md](../docs/reviews/WEB_SALES_4_STRIPE_CONTRACTS.md)
  - ★active化の根拠は**署名検証済みWebhookでの支払い成功のみ**。success_url到達は根拠にしない
  - 料金は14番/15番の実測で確定（基本料金に管理者1名を含む→追加席=契約人数−1／1人20,000・3人26,000・5人32,000）。金額・数量・Price IDはすべてサーバー側決定
  - `billing_cycle_anchor`に翌月1日0:00 JSTを渡し初回日割り・以後毎月1日。日割り額はStripeのprorationに委ね自前計算しない
  - contract_started_atはCOALESCEで守り**初回の支払い成功時のみ**設定。再送で上書きしない
  - 冪等性は`stripe_event_id`のUNIQUEで「記録できた側だけが処理」。並行4件でも処理1回・順序逆転はSTALE_EVENTで上書きしない
  - metadataだけで会社を信用せずDBのStripe IDと突合（他社すり替え不可）
...
- **WEB-SALES-5B** `99cda82` 契約状態に応じた画面制御。詳細は[docs/reviews/WEB_SALES_5B_CONTRACT_GATING.md](../docs/reviews/WEB_SALES_5B_CONTRACT_GATING.md)
  - 支払い前・支払い失敗・解約後は業務メニューを出さず、直接URLも**許可リスト（`/`と`/settings`の完全一致）**でご案内へ戻す
  - ★取得前・取得失敗をactive扱いにしない（業務メニューが一瞬見えない）。契約状態の問い合わせはContextの1か所へ集約
...
- **WEB-SALES-6** `f63fc8c` 解約予約・予約取消・課金操作のレート制限。詳細は[docs/reviews/WEB_SALES_6_CANCELLATION.md](../docs/reviews/WEB_SALES_6_CANCELLATION.md)
  - 解約は**即時ではなく現在の請求期間の終了時**。予約後も期間終了までは通常利用でき、日割り返金・自動返金は行わない。期間終了前なら予約を取り消せる
  - `POST`/`DELETE /api/billing/cancellation-schedule` を新設。**Stripeへ送るのは `cancel_at_period_end` ただ1つ**（即時解約・返金・Price・数量・按分に触れない）
  - ★**canceled へ移すのは署名検証済みの `customer.subscription.deleted` だけ。** `subscription.updated` では解約予約と期間終了日時を記録するのみで契約状態は `active` のまま。`canceled_at` は初回のみ記録するよう是正
  - migration 021 で `cancellation_scheduled_at` を**1列だけ**追加。予約の有無は日時のNULL判定で表し真偽値を持たない。`contract_end_at`（運営者の停止予定日・`status='active'`で空へ戻る）は再利用しない
  - 課金4経路（Checkout・Portal・解約予約・予約取消）に**サーバー側レート制限**。既存 `login_attempts` を scope 分離で再利用し、会社10回／利用者5回／10分・429＋Retry-After。権限確認の後に置き、一般利用者の連打で管理者の枠を潰せないようにした
  - 画面は `/settings` の既存セクションへ追加（新URLなし）。確認必須・解約予定日と返金なし・データ非削除を明示・連打1要求・反映確認は最大3回で打ち切り
  - 契約状態の再取得で画面が作り直され、手続き直後に設定画面が初期タブへ戻る不具合を是正（ブラウザ実測で検出）
  - テスト1645件成功・失敗0・skip1（新規206件／既存の削除・skip追加・緩和0）。`npm run build`成功。実Stripe通信0・実決済0・実Webhook0・実メール0・本番DB接続0
- **本番公開はNo-Go継続**（特商法表記・利用規約の契約条項・キャンペーン規約が未整備／本番Webhook未登録／再契約・契約人数変更が未実装）
...
| **v6.7** | 2026-07-15 | Claude Code(CEO追加指示による) | **経営計画書 Version 1.0(社内検討用)を新規作成。** Project Bible Version 7.1→7.2に更新。CEO指示「銀行提出版とは別に、Partner Program・9プラン・継続課金・紹介ネットワークを考慮した実際の経営目標としての経営計画版を作成し、費用を固定費・変動費(特にサーバー費・AI API費・Partner Program報酬を分離)へ分類、利益率も再計算してほしい」に基づき新設。銀行提出版とは明確に別の社外非公開文書(表紙に「CONFIDENTIAL — 社内検討用」と明記)。全9章：本書の位置づけ(銀行提出版との対比表)／9プラン価格グリッド(Small/Medium/Enterprise×Lite/Standard/Premium)／顧客ミックスと想定ARPU(Year1 ¥50,000→Year2 ¥68,000→Year3 ¥85,000)／売上計画(経営目標、550万円→2,900万円→7,200万円、銀行提出版との比較表併記)／コスト構造:固定費(人件費・基本管理費)／コスト構造:変動費(サーバー費・AI API費・Partner Program報酬・その他変動費を分離)／利益計画・利益率(△250万円[△45.5%]→1,060万円[36.6%]→3,330万円[46.3%]、Year2でPartner Program稼働により黒字化)／まとめ。QA時に発見した(1)本書の位置づけ章の比較表とフッターの衝突、(2)利益計画グラフでYear3の営業利益額と営業利益率のデータラベルが重なり判読不能になる不具合、の2件を行間隔圧縮・第2軸表示範囲調整でそれぞれ解消 |
...
| **v8.3** | 2026-08-10 | Claude Code(代表指示による・WEB-SALES-6) | **顧客自身による解約（期間終了時解約）と課金操作のレート制限を実装（SmartLabo側は文書のみ。本番変更0）。** ①解約は**即時ではなく現在の請求期間の終了時**。予約後も期間終了まで通常利用でき、日割り返金・自動返金は行わず、期間終了前なら取り消せる ②`POST`/`DELETE /api/billing/cancellation-schedule` を新設し、**Stripeへ送るのは `cancel_at_period_end` ただ1つ**（即時解約・返金・Price・数量・按分に触れない。解約予定日はStripeが返した値が正本で自前計算しない）③★**canceled へ移すのは署名検証済みの `customer.subscription.deleted` だけ**。`subscription.updated` では解約予約と期間終了日時を記録するのみで契約状態は `active` のまま維持し、`status=active` を有効化の根拠にもしない ④`canceled_at` を初回のみ記録するよう更新式を是正（`COALESCE(canceled_at, ?)`）⑤migration 021 で `cancellation_scheduled_at` を**1列だけ**追加。予約の有無は日時のNULL判定で表して真偽値を二重に持たず、運営者の停止予定日である `contract_end_at`（`status='active'`で空へ戻される）は再利用しない ⑥課金4経路にサーバー側レート制限を追加。**既存 `login_attempts` を scope 分離で再利用**し新テーブルを作らず、キーは会社ID／会社ID＋利用者ID（IPは使わない）、会社10回・利用者5回・10分窓、429＋Retry-After。権限確認の後に置き、一般利用者の連打で管理者の枠を潰せない設計にした。時計を注入できるようにし、テストは待たずに窓明けを検証 ⑦画面は `/settings` の既存セクションへ追加（新URLなし）。押しただけでは実行せず、期間終了時解約・利用継続・返金なし・データ非削除・解約予定日を確認画面で明示。連打は1要求、反映確認は最大3回・2秒間隔で打ち切り ⑧契約状態の再取得のたびにレイアウトが画面を作り直し、手続き直後に設定画面が初期タブへ戻る不具合をブラウザ実測で検出し是正。記録=[docs/reviews/WEB_SALES_6_CANCELLATION.md](../docs/reviews/WEB_SALES_6_CANCELLATION.md)。テスト1645件成功・失敗0・skip1（新規206件・既存の削除/skip追加/緩和0・customer-portalは意味を保った1対1更新で161→162件）。**判定: WEB-SALES-6S（Stripeテストモードでの解約実機確認）はGo、本番公開はNo-Go継続（法務3点未整備・本番Webhook未登録・再契約/契約人数変更が未実装）** |
