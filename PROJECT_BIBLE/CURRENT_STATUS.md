...
| Current Task | **①ホームページ：Homepage v1.0.0として`https://smartlaboworks.com/`で正式公開完了**（会社概要の代表者名・設立年月・事業内容を反映、所在地・電話番号はCEO指示により非掲載、問い合わせフォームをmailtoへ一時差し替え。AIチャット実API接続・問い合わせフォーム本番稼働・Google Analytics・Search ConsoleはVersion1.1で対応するとCEOが確定）。**GitHub Pages側のカスタムドメイン設定・DNS移行(Aレコード4件・wwwのCNAME)・HTTPS強制はCEOご自身により2026-07-16に完了、公開確認済み**（XServerへのフォームPHPアップロード・reCAPTCHA設定・送信テストはVersion1.1でフォーム再公開時に実施）。**②財務計画：銀行提出用事業計画書Version1.4は2026-07-15のCEO指示「1のままで資料出力して」により選択肢1(代表者報酬を含まない試算のまま)で確定・提出用ファイルを出力済み**([DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx)／[.pdf](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pdf)、詳細は下記「銀行提出用事業計画書 Version 1.4」参照)。経営計画書Version1.0は旧解釈のまま未更新でv1.4との前提不整合が残存、CEO確認待ち |
| Next Task | ①公開済みのホームページについて、Version1.1で[62_CEO_Publish_Guide.md](62_CEO_Publish_Guide.md)の残りステップ(XServerアップロード→config.php設定→reCAPTCHA設定→送信テスト)を実施し問い合わせフォームを本番稼働へ。Gitタグ`homepage-v1.0.0`・GitHub Releaseの作成はCEOの明示的な指示を得てから実施。Version1.1でAIチャット実API接続・Google Analytics・Search Consoleにも対応予定([63_Post_Launch_Roadmap.md](63_Post_Launch_Roadmap.md)参照)。②銀行提出版v1.4は代表者報酬の扱いが確定したため、所在地・資本金・代表者略歴の記入(CEO記入待ち)のみ残課題。実際の銀行提出時はファイル名に「_submitted」を付与([DOCUMENT/FINANCE/README.md](../DOCUMENT/FINANCE/README.md)のVersion管理ルール)。経営計画書Version1.0を新しい費用構造・新規/更新モデルへ合わせて更新するか要CEO判断。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち([63_Post_Launch_Roadmap.md](63_Post_Launch_Roadmap.md)優先度②) |
| Last Update | 2026-07-16(公開完了・所在地/電話番号削除反映) |
