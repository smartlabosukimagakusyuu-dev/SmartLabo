...
```
Project Bible Version：5.2
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.2(情報量整理・製品画面主役化。セクション11→7、背景ブロック11→5。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/)
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：ホームページの情報量整理(CEO承認2026-07-12)。業種展開・料金セクション削除、AI社員と社長の1日を統合、画面ショーケースを大型2枚に再設計。pushはCEO確認後
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | **2.2 — GitHub Pagesで公開中**([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。CEO承認「情報量整理・製品画面主役化」により、セクション数11→7・背景ブロック11→5へ整理。詳細は下記「ホームページ情報量整理」を参照 |
...
| Current Task | **ホームページ情報量整理完了。** 「業種展開」「料金」セクションを削除(準備中表示のみだったため非表示化)、「AI社員」と「社長の1日」を統合、画面ショーケースをDashboard・不動産AIの大型2枚構成へ再設計。Heroの情報量削減・CTAを「デモを見る」「導入相談」の2つに統一 |
| Next Task | ホームページ3コミット分のpushはCEO確認後。営業テンプレート／Appointment Templateの追加分（followup/thanks/contract等）・他業種テンプレートの追加、および将来のSmart AI Router公開API化はCEO承認後に着手 |
