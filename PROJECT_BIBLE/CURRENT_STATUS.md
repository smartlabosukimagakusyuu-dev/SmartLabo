...
```
Project Bible Version：7.7
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：v1.0.0（2026-07-16 CEO最終公開指示により正式公開版として確定。代表者名・設立年月・事業内容をCEOより取得し会社概要ページへ反映(所在地・電話番号は引き続き未確定)。問い合わせフォームは本番送信テスト未完了のため一時的に撤去し、info@smartlaboworks.comへのmailtoリンクへ差し替え。AIチャット実API接続・問い合わせフォーム本番稼働・Google Analytics・Search ConsoleはVersion1.1で対応する方針をCEOが確定）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/。**正式ドメイン`smartlaboworks.com`は2026-07-16時点でDNS実測の結果、GitHub PagesではなくXserverの初期設置ページを指していることを確認(A/CNAMEレコード未切替)。GitHubリポジトリ側のカスタムドメイン設定・DNS切替はCEOアクション待ち(詳細は[62_CEO_Publish_Guide.md](62_CEO_Publish_Guide.md))**
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：2026-07-16、CEOより「銀行口座開設・融資申込・会社実態の公開」を目的とした最終公開指示を受け、ホームページをHomepage v1.0.0として確定・コミット・pushを実施(詳細は下記「Homepage v1.0.0 正式公開」参照)。銀行提出用事業計画書Version1.4は**CEO承認により選択肢1(代表者報酬を含まない試算のまま提出)で確定**([DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx)／[.pdf](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pdf)、提出用ファイルとして出力済み)。経営計画書Version1.0は旧解釈のまま未更新で本書との前提不整合が残存、CEO確認待ち。代表者名(小川昌利)・設立年月(2026年7月)・事業内容はCEO確認済みで反映済み。所在地・資本金・代表者略歴は引き続き「CEO確認待ち」表示のまま
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | **v1.0.0**（2026-07-16、CEO最終公開指示により正式公開版として確定。会社概要ページに代表者名・設立年月・事業内容を反映、問い合わせフォームはmailtoリンクへ一時差し替え）。GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。正式ドメイン`smartlaboworks.com`は2026-07-16のDNS実測でXserver初期設置ページを指しており、GitHub Pages側のカスタムドメイン切替・DNS切替は未実施(CEOアクション待ち) |
...
| Current Project | Company Setup → **Homepage v1.0.0 正式公開（2026-07-16 CEO最終公開指示）** |
| Current Task | **①ホームページ：Homepage v1.0.0として正式公開版を確定・コミット・push実施済み**（会社概要の代表者名・設立年月・事業内容を反映、問い合わせフォームをmailtoへ一時差し替え。AIチャット実API接続・問い合わせフォーム本番稼働・Google Analytics・Search ConsoleはVersion1.1で対応するとCEOが確定）。**GitHub Pages側のカスタムドメイン切替・DNS切替・XServerへのフォームアップロードはCEO(または担当者)による実施が残課題**（[62_CEO_Publish_Guide.md](62_CEO_Publish_Guide.md)参照、2026-07-16のDNS実測で未着手を確認）。**②財務計画：銀行提出用事業計画書Version1.4は2026-07-15のCEO指示「1のままで資料出力して」により選択肢1(代表者報酬を含まない試算のまま)で確定・提出用ファイルを出力済み**([DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pptx)／[.pdf](../DOCUMENT/FINANCE/SmartLabo_BusinessPlan_v1.4.pdf)、詳細は下記「銀行提出用事業計画書 Version 1.4」参照)。経営計画書Version1.0は旧解釈のまま未更新でv1.4との前提不整合が残存、CEO確認待ち |
| Next Task | ①CEOによる[62_CEO_Publish_Guide.md](62_CEO_Publish_Guide.md)の実施(GitHub Pages切替→DNS確認→XServerアップロード→config.php設定→reCAPTCHA設定→送信テスト→公開確認→公開後確認)。完了後、Gitタグ`homepage-v1.0.0`・GitHub Releaseの作成はCEOの明示的な指示を得てから実施。Version1.1でAIチャット実API接続・問い合わせフォーム本番稼働・Google Analytics・Search Consoleに対応予定([63_Post_Launch_Roadmap.md](63_Post_Launch_Roadmap.md)参照)。②銀行提出版v1.4は代表者報酬の扱いが確定したため、所在地・資本金・代表者略歴の記入(CEO記入待ち)のみ残課題。実際の銀行提出時はファイル名に「_submitted」を付与([DOCUMENT/FINANCE/README.md](../DOCUMENT/FINANCE/README.md)のVersion管理ルール)。経営計画書Version1.0を新しい費用構造・新規/更新モデルへ合わせて更新するか要CEO判断。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち([63_Post_Launch_Roadmap.md](63_Post_Launch_Roadmap.md)優先度②) |
