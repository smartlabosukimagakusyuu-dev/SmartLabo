...
Current Task：Smart Labo Worksの正式コードベースを`smartlabo-works`に一本化(2026-07-10 CEO決定)。WEBSITE/app.htmlはデモサイト／マーケティング用プレビューへ位置づけ変更
```
...
| Current Task | Smart Labo Worksの正式コードベースを`smartlabo-works`に一本化(2026-07-10 CEO決定)。`WEBSITE/app.html`をデモサイト／マーケティング用プレビューへ位置づけ変更し、関連ドキュメント(PROJECT_BIBLE・`smartlabo-works/PRODUCT_REQUIREMENTS.md`・`PRODUCT_BOUNDARY.md`)を統一 |
| Next Task | `smartlabo-works`側でCompany OS機能(Smart Growth・人材育成等)とSmart Labo Works機能の実装分離に着手 / Company Brain実データ連携 / Homepageのフォーム機能・法務ページ整備 / 正式ロゴ展開 / Xserverサーバー契約完了後に正式ドメインへ切り替え |
