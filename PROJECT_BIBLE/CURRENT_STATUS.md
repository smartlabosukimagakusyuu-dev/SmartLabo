...
```
Project Bible Version：5.7
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.5（ロゴ露出強化：全ページヘッダーロゴ拡大・巨大透かしロゴ・Powered by追加）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：Smart Labo Works料金体系「Pricing Philosophy」を正式制定(CEO指示2026-07-12)。①利用規模×②プラン×③オプションの3要素構造をPROJECT_BIBLE 12章として新設し、ホームページ・営業資料・見積書・契約書等の共通仕様に。Company OS「Pricing Manager」は設計のみ、実装は別Task承認待ち
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.5 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。今回の更新はホームページではなくPROJECT_BIBLE本体(新章12_Pricing_Philosophy.md)が対象 |
...
| Current Task | **Pricing Philosophy正式制定完了。** 新章[12_Pricing_Philosophy.md](12_Pricing_Philosophy.md)を新設し、①利用規模(Small/Medium/Enterprise)×②プラン(Lite/Standard/Premium)×③オプション(7種)の3要素構造と「料金ではなく導入プランとして表現する」思想を制定。ホームページ・Product Book・Company OS・営業資料・見積書・契約書への共通適用を明記。Company OS「Pricing Manager」(利用規模→プラン→オプション選択で料金表/見積書/契約書/営業資料を自動生成)は設計のみ記載し、実装はsmartlabo-works側の別Task承認待ちと明記 |
