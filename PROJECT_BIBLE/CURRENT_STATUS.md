...
```
Project Bible Version：6.1
Brand Version：5.1
Design Bible Version：2.3
Homepage Version：2.8（ホームページ本体の見た目に変更なし。WEBSITE/api/配下に問い合わせフォーム送信バックエンドを新規実装、フロントエンドとの接続はStep5）。GitHub Pagesで公開中: https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/
Smart Labo Works：正式コードベースは別リポジトリ`smartlabo-works`(Node.js版)。詳細Versionは同リポジトリの`PRODUCT_REQUIREMENTS.md`を参照
Current Task：正式リリース前 最終整備(CEO承認2026-07-14)のStep1〜4完了。会社概要ページ・法務ページドラフト・問い合わせフォーム送信基盤(Vercel Serverless Functions想定)を実装。ローカル検証でCSRF検証の脆弱性を発見・修正済み。Vercelアカウント作成/reCAPTCHA登録/通知先メール確定等はCEOアクション待ちのためStep5(contact.html実接続)は未着手
```
...
| Brand Version | 5.1 |
| Design Bible Version | 2.3 |
| Homepage Version | 2.8 — GitHub Pagesで公開中([https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/](https://smartlabosukimagakusyuu-dev.github.io/SmartLabo/))。見た目の変更なし。`WEBSITE/api/`に問い合わせフォーム送信バックエンドを新規実装（未デプロイ、フロント接続前） |
...
| Current Task | **正式リリース前 最終整備 Step1〜Step4完了。** Step1：会社概要ページ新設。Step2：法務ページドラフト化。Step3：送信基盤設計確定（Vercel Serverless Functions推奨、CEO承認）。Step4：`WEBSITE/api/`にバックエンド実装（CSRF・レート制限・reCAPTCHA v3・入力検証・メール送信）。ローカル検証でCSRF検証の脆弱性を発見・修正済み |
| Next Task | Step5(`contact.html`/`js/pages.js`の実送信対応)はCEOアクション待ち：Vercelアカウント作成・環境変数設定、Google reCAPTCHA管理画面でのサイト登録、通知先/送信元メールアドレスの確定が前提。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち |
