...
| Dashboard Version | **1.0(2026-07-07完成。GitHub Pages公開中。全9ページ: Dashboard/CompanyBrain/AI/顧客管理CRM/物件管理/契約管理/広告集客/レポート/設定。デモデータ: 顧客5件・契約7件・物件6件)**⚠️ |
| Smart Labo Works Version | **1.0(営業・デモ・テスト導入可能な品質。2026-07-07完成)**⚠️ |
...
⚠️ **未確認の重要な差異(2026-07-10 マージ時に発覚)：** 上記のDashboard Version 1.0／Smart Labo Works Version 1.0 は、本リポジトリ内`WEBSITE/app.html`(GitHub Pages公開・静的サイト・デモデータ)を指す。一方、`PRODUCT_BOUNDARY.md`・`PRODUCT_REQUIREMENTS.md`（別リポジトリ`smartlabo-works`）が調査対象としたのは`smartlabo-works/app.html`（Node.jsサーバー・実際のOpenAI/Claude API連携あり）であり、**両者はコードベースが異なる別実装**。同じ「Smart Labo Works」という名前で2つの異なるv1.0が並行して存在している状態であり、どちらを正式な顧客提供版とするか、または統合するのかはCEO確認が必要な未決定事項。旧稿にあった「Company OS完成」という表現(2026-07-07時点、CEOレビュー前の記述)は、2026-07-10の製品境界決定(「Company OSは社内専用・非売品」)と矛盾するため本行から削除した。
