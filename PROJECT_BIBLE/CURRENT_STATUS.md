...
Current Task：61_Release_Checklist.mdを「v1.0 Release Checklist」として全面拡充(18項目)。✅ローカル確認済み9／⏸本番環境が必要6／❓CEO判断・作業が必要3(Search Console登録・Analytics導入要否・GitHub Release)。既知の制限事項(Known Issues)10項目を整理。全項目確認完了後に「v1.0 Release」として提出予定
```
...
| Current Task | **v1.0 Release Checklistへ全面拡充。** [61_Release_Checklist.md](61_Release_Checklist.md)をCEO指定の18項目(ホームページ/問い合わせフォーム/SSL/メール送受信/スマホ表示/PC表示/SEO/OGP/favicon/robots.txt/sitemap.xml/404ページ/Google Search Console登録/Google Analytics登録/PageSpeed/GitHub Release/更新履歴/既知の制限事項)へ再編。✅ローカル確認済み9／⏸本番環境が必要6／❓CEO判断・作業が必要3。既知の制限事項10項目を整理 |
| Next Task | CEOによる`config.php`本番設定・`public/`一式のXServerアップロード・GitHub Pagesカスタムドメイン設定を経て、⏸項目(SSL/問い合わせ/メール/PageSpeed)の本番確認を実施。Google Search Console登録・Google Analytics導入要否はCEO判断待ち。全項目確認後、CEO承認を得てGitタグ`homepage-v1.0.0`・GitHub Releaseを作成し「v1.0 Release」として提出。Company OS「Pricing Manager」の実際のコード実装は別Task承認待ち |
