# intake-api — 店舗向けHP導入フォームの受付API（HP-ONBOARDING-4B）
...
店舗入力画面（4C）／Google Drive 実連携／通知メールの実送信／
管理画面とその認証（4D）／Smart Labo Operations との連携（OPS-4）／
Stripe（一切）／`submission_id` による二重送信の重複排除（§8 参照）

## 8. 残っている設計上の宿題

`submission_id` による冪等化は **SSOT v1.2 に保存列が無い**ため未実装。
4B では SSOT §6.4 の「status による冪等化」で二重送信を防いでいる
（2回目の提出は状態を変えず、履歴も増やさない）。
`submission_id` を永続化するなら `intake_submission_history` へ列追加が要るため、
**先に SSOT を改定してから**実装する。
